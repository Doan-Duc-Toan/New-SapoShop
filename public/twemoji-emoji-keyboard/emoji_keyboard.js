...
class EmojiKeyboard {
    ...
    click_on_category(kb, event, stay) {
        // Lấy phần tử mục tiêu dựa trên việc click vào path hoặc trực tiếp vào phần tử
        let target = event.target.closest('svg, .category-item'); // Sử dụng closest để hỗ trợ click vào bất kỳ phần nào của SVG hoặc phần tử danh mục
    
        // Xóa lớp 'selected' từ tất cả các phần tử đã chọn trước đó
        document.querySelectorAll('#emojikb-leftlist > svg.selected').forEach(svg => {
            svg.classList.remove('selected');
        });
    
        // Thêm lớp 'selected' vào phần tử mục tiêu
        target.classList.add('selected');
    
        if (!stay) {
            // Cuộn đến phần tử danh mục đã chọn
            const categ = target.dataset.emojikb_categ;
            const targetCategory = document.querySelector(`div.emojikb-categname[data-emojikb_categ="${categ}"]`);
    
            if (targetCategory) {
                targetCategory.scrollIntoView({ behavior: 'smooth', block: 'start' }); // Cuộn sao cho tiêu đề nằm ở trên cùng của viewport
            }
        }
    }
