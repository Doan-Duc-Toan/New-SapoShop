     ...
     @if (!empty($products->first()))
        <div id="filter">
            <ul class="list-filter">
                <li class="fr-price list-filter-item">
                    <form action="{{ route('client.filter_products', $type) }}" method="POST">
                        @csrf
                        <span>Lọc giá (Triệu đồng)</span>
                        <input name="minimum_price" type="number" min="0"
                            value="@php
if(!empty($min))echo $min/1000000 @endphp" placeholder="Giá tối thiểu">
                        <input name="maximum_price" type="number" value="@php
if(!empty($max))echo $max/1000000 @endphp"
                            min="0" placeholder="Giá tối đa">
                        <button name="btn_apply">Áp dụng</button>
                    </form>
                </li>
                <li class="list-filter-item">
                    ...
                    <div>
                        @foreach ($suppliers as $supplier)
                            <a class="filter-item"
                                href="{{ route('client.search_key', [$type, 'supplier', $supplier]) }}">{{ $supplier }}</a>
                        @endforeach
                    </div>
                </li>
                <li class="list-filter-item">
                    ...
                    <div>
                        @foreach ($cats as $cat)
                            <a class="filter-item"
                                href="{{ route('client.search_cat', [$type, $cat->name]) }}">{{ $cat->name }}</a>
                        @endforeach
                    </div>
                </li>
                <li class="list-filter-item">
                    ...
                    <div>
                        <a class="filter-item"
                            href="{{ route('client.search_key', [$type, 'specifications', 'Sạckhôngdây']) }}">Sạc không
                            dây</a>
                        <a class="filter-item"
                            href="{{ route('client.search_key', [$type, 'specifications', 'vântay']) }}">Bảo mật vân
                            tay</a>
                        <a class="filter-item"
                            href="{{ route('client.search_key', [$type, 'specifications', 'khuônmặt']) }}">Nhận diện
                            khuôn mặt</a>
                        <a class="filter-item"
                            href="{{ route('client.search_key', [$type, 'specifications', 'Kháng']) }}">Kháng
                            nước, kháng bụi</a>
                        <a class="filter-item" href="{{ route('client.search_key', [$type, 'specifications', '5G']) }}">Hỗ trợ
                            5G</a>
                    </div>
                </li>
            </ul>
        ...
        <div id="arrange">
            <div id="arrange_option">
                <span class="center">Sắp xếp theo:</span>
                <a class="filter-item" href="{{ route('client.sort', [$type, 'Default']) }}">Mặc định</a>
                <a class="filter-item" href="{{ route('client.sort', [$type, 'Asc']) }}">Giá tăng dần</a>
                <a class="filter-item" href="{{ route('client.sort', [$type, 'Desc']) }}">Giảm dần</a>
                <a class="filter-item" href="{{ route('client.sort', [$type, 'Newest']) }}">Mới nhất</a>
            </div>
